...
async def logo(bot, message):
  try:
    ...
    await message.reply_photo(logo,caption="**🔥 Logo Generated By ᴇᴘɪᴄ ʙᴏᴛs 🇱🇰**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
...
async def logo(bot, message):
  try:
    ...
    await x.edit("`🔄 Done Generated...Epic Team logo Sending You`")
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By </ᴇᴘɪᴄ ʙᴏᴛs <s/ʟ>🇱🇰**")
  except FloodWait:
    pass
  except Exception as e:
    try:
